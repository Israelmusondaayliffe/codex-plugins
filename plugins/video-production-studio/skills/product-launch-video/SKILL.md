...
You make the one judgment call — **which preset**. Read `the HyperFrames companion skill hyperframes-creative and its design specification` and pick the preset whose look best fits the brand and brief. Then run:
...
The audio script handles narration, word timings, BGM lookup from HeyGen's music library, and timing metadata. BGM mood comes from the storyboard's `music:` field. This uses the HeyGen Audio API for retrieval, not generation, and uses the same `~/.heygen` credential as TTS. For provider details, read `the HyperFrames companion skill hyperframes-media and its TTS guidance`.
...
Read `references/visual-design.md`, `references/composition.md`, `references/motion-language.md`, and `the HyperFrames companion skill hyperframes-animation`. Use `visual-design.md` for required frame fields and the required `## Video direction` block. Use `composition.md` for layout, hierarchy, focal points, and visual roles. Use `motion-language.md` and `the HyperFrames companion skill hyperframes-animation` for valid effects and blueprint IDs. Do not invent effect names or blueprint IDs.
...
| `HyperFrames companion capability`                           | Step 3.1: choose or understand TTS providers and voices. |
