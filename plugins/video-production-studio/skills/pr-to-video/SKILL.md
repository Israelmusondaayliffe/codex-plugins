...
Read `references/story-design.md`, `the HyperFrames companion skill hyperframes-core and its storyboard format`, and `the HyperFrames companion skill hyperframes-core and its script format`. Use them to write `STORYBOARD.md` and, when narration is needed, `SCRIPT.md`.
...
The audio script handles narration, word timings, BGM lookup from HeyGen's music library, and timing metadata. BGM mood comes from the storyboard's `music:` field. This uses the HeyGen Audio API for retrieval, not generation, and the same `~/.heygen` credential as TTS. For provider details, read `the HyperFrames companion skill hyperframes-media and its TTS guidance`.
...
Read `references/visual-design.md`, `references/composition.md`, `references/motion-language.md`, `references/code-vocabulary.md`, and `the HyperFrames companion skill hyperframes-animation`. Use `visual-design.md` for required frame fields and the required `## Video direction` block, and for how a code beat names a `code-*` block as its `focal`. Use `code-vocabulary.md` to pick the right block per beat (diff = `code-diff`, refactor = `code-morph`, new code = `code-typing`, …). Use `composition.md` for layout/hierarchy/focal points and `motion-language.md` + `the HyperFrames companion skill hyperframes-animation` for valid effect and blueprint IDs. Do not invent effect names or block/blueprint IDs.
...
Before dispatch, read `sub-agents/frame-worker.md` and `the HyperFrames companion skill hyperframes-core and its dispatch guidance`. Dispatch one sub-agent per frame, in parallel if possible; otherwise run workers in waves. Each worker gets exactly one frame. Each worker's context must include `PROJECT_DIR`, `frame_id`, canvas size, caption status and keep-out band if captions are enabled, `ANIM_DIR` (absolute path to `the HyperFrames companion skill hyperframes-animation`), and the absolute path to `references/code-vocabulary.md`. Each worker reads `frame.md`, its own `## Frame N` block, the recipe body for each cited effect/blueprint ID, and — for a code beat — `code-vocabulary.md` for the named block's inputs. Each worker writes only `compositions/frames/NN-*.html`; workers never edit `STORYBOARD.md`.
...
| `HyperFrames companion capability`     | Step 3: write `STORYBOARD.md`.                              |
| `HyperFrames companion capability`             | Step 3: write `SCRIPT.md`.                                  |
| `HyperFrames companion capability`                               | Step 3.1: choose or understand TTS providers.               |
...
| `HyperFrames companion capability`                                                         | Step 4: cite effect and blueprint IDs.                      |
| `[sub-agents/frame-worker.md](sub-agents/frame-worker.md)`                                                       | Step 5: dispatch per-frame workers.                         |
| `HyperFrames companion capability`     | Step 5: dispatch sub-agents safely.                         |
| `HyperFrames companion capability` | Step 2: the claude preset (fixed style).                    |
