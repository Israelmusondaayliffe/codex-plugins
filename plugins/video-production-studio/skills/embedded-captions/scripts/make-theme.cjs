...
function setpieceCoverword() {
  ...
  const bundledFont = path.join(SKILL, "assets/brand/CyberpunkReplica.ttf");
  const fontPath = process.env.CYBERPUNK_REPLICA_FONT_PATH || bundledFont;
  if (!fs.existsSync(fontPath)) {
    throw new Error(
      "The coverword setpiece needs a separately licensed CyberpunkReplica.ttf. " +
        "Set CYBERPUNK_REPLICA_FONT_PATH to your local font file.",
    );
  }
