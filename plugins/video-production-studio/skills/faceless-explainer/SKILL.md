...
You make the one judgment call — **which preset**. Read `the HyperFrames companion skill hyperframes-creative and its design specification` and browse `the HyperFrames companion skill hyperframes-creative and its frame presets`; pick the preset whose look best fits the topic, tone, and audience. Then run:
...
Read `references/visual-design.md`, `references/composition.md`, `references/motion-language.md`, and `the HyperFrames companion skill hyperframes-animation`. Use `visual-design.md` for required frame fields and the required `## Video direction` block. Use `composition.md` for layout, hierarchy, focal points, and the invented-visual treatment. Use `motion-language.md` and `the HyperFrames companion skill hyperframes-animation` for valid effects and blueprint IDs. Do not invent effect names or blueprint IDs.
...
Before dispatch, read `sub-agents/frame-worker.md` and `the HyperFrames companion skill hyperframes-core and its dispatch guidance`. Dispatch one sub-agent per frame, in parallel if possible; otherwise run workers in waves. Each worker gets exactly one frame.

Each worker context must include `PROJECT_DIR`, `frame_id`, canvas size, caption status and keep-out band if captions are enabled, and `ANIM_DIR` as the absolute path to `the HyperFrames companion skill hyperframes-animation`. Each worker reads `frame.md`, its own `## Frame N` block from `STORYBOARD.md`, and the recipe body for each cited effect or blueprint ID. Each worker writes only `compositions/frames/NN-*.html`. Workers must never edit `STORYBOARD.md`.
...
| Read                                                                                                         | When                                          |
| ------------------------------------------------------------------------------------------------------------ | --------------------------------------------- |
| `HyperFrames companion capability`                           | Step 2: choose and adopt a frame preset.      |
| `HyperFrames companion capability`     | Step 2: apply brand tokens correctly.         |
